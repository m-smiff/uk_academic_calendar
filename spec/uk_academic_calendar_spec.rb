...
RSpec.describe UKAcademicCalendar do
  ...
  describe '::all_terms' do
    let(:all_terms) { described_class.all_terms 2023 }

    it do
      expect(all_terms).to be_instance_of(Array)
                       .and have_exactly(3).items
                       .and all be_a described_class::Term
    end
  ...
  describe '::term_including' do
    ...
    context 'when date == 2021-09-01' do
      let(:date) { Date.new(2021, 9, 1) }

      it_behaves_like 'is an Autumn Term', 2021
    end

    context 'when date == 2021-12-31' do
      let(:date) { Date.new(2021, 12, 31) }

      it_behaves_like 'is an Autumn Term', 2021
    end

    context 'when date == 2022-01-01' do
      let(:date) { Date.new(2022, 1, 1) }

      it_behaves_like 'is a Spring Term', 2021
    end

    context 'when date == 2022-04-17 (Easter Sunday)' do
      let(:date) { Date.new(2022, 4, 17) }

      it_behaves_like 'is a Spring Term', 2021
    end

    context 'when date == 2022-04-18 (Easter Monday)' do
      let(:date) { Date.new(2022, 4, 18) }

      it_behaves_like 'is a Summer Term', 2021
    end

    context 'when date == 2022-08-31' do
